  ...
  const datos = useMemo(
    () =>
      visibles.map((f) => ({
        ...
        // Solo el correo de quien lo cargó desde la web. Lo que llega de las
        // apps no trae email: su autoría es un uid y resolver el nombre en
        // `usuarios_public` costaría una lectura por persona.
        subidoPorTexto: f.registradoPor || "",
      })),
    [visibles]
  );
  ...
  const columnas = useMemo(
    () => [
      ...
      },
      {
        ...
        size: 220,
        Cell: ({ cell }) => {
          const correo = cell.getValue();
          if (!correo) return <Typography variant="body2" color="text.disabled">—</Typography>;
          // Con el tooltip se lee entero aunque la columna lo corte.
          return (
            <Tooltip title={correo}>
              <Typography variant="body2" noWrap>{correo}</Typography>
            </Tooltip>
          );
        },
      },
