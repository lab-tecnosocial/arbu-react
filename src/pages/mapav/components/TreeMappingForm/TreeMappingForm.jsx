...
export const TreeMappingForm = ({ onSubmit }) => {
  ...
  const { showTreeMappingForm } = useSelector((state) => state.mapa);
  ...
  console.log("showTreeMappingForm", showTreeMappingForm);
    ...
    <div className={styles.field}>
      ...
      {previews[field] && (
        <div className={styles.previewContainer}>
          <img src={previews[field]} alt={`Preview ${label}`} className={styles.preview} />
          <button type="button" onClick={() => removeImage(field)} className={styles.removeIcon}><X size={16} /></button>
        </div>
      )}
  ...
  if (!showTreeMappingForm) return null;

  return (
    <form onSubmit={handleSubmit} className={styles.form}>
      <h2 className={styles.title}>Formulario de Mapeo</h2>

      {renderFileInput('Árbol completo', 'arbolCompleto', true)}
      {renderFileInput('Raiz o base', 'raizBase')}
      {renderFileInput('Corteza', 'corteza')}
      {renderFileInput('Hoja', 'hoja')}
      {renderFileInput('Flor', 'flor')}

      {renderTextInput('Proyecto', 'proyecto', 'Ingrese el proyecto')}
      {renderTextInput('Nombre propio', 'nombrePropio', 'Ingrese nombre propio')}
      {renderTextInput('Nombre común', 'nombreComun', 'Ingrese nombre común')}
      {renderTextInput(
        "Nombre científico *",
        "nombreCientifico",
      ...
      {renderTextInput('Altura (m)', 'altura', 'Ingrese altura en metros')}
      {renderTextInput('Diámetro a la altura del pecho (cm)', 'diametro', 'Ingrese diámetro en cm')}
